...
function updateBreadcrumb(page) {
  ...
  let html = '';
  if (page === 'home') { bc.innerHTML = ''; return; }
  if (page === 'courses') { html = '<span>Courses</span>'; }
...
async function loadClasses() {
  ...
  if (!ok) { grid.innerHTML = `<p style="color:var(--accent2);padding:2rem">Failed to load classes.</p>`; return; }
...
async function loadChaptersPage(cls) {
  ...
  document.getElementById('chaptersTitle').textContent = `Class ${cls.number} — Chapters`;
  document.getElementById('chaptersSubtitle').textContent = `Subject choose karo aur chapter pe click karo.`;
  navigate('chapters');
  ...
  if (subjects.length) loadChapters(subjects[0], tabs.querySelector('.subject-tab'));
}
...
async function loadChapters(subject, tabEl) {
  ...
  const { ok, data } = await apiFetch(`/chapters?classNumber=${state.currentClass.number}&subject=${encodeURIComponent(subject)}`);
  if (!ok) { grid.innerHTML = `<p style="color:var(--accent2);padding:2rem">Failed to load chapters.</p>`; return; }

  const chapters = data.data;
  const completedIds = state.progress.filter(p => p.completed).map(p => p.chapterId?._id || p.chapterId);
  ...
  }).join('') || `<p style="color:var(--text2);padding:2rem">No chapters found for this subject yet.</p>`;
}
...
async function loadVideosPage(chapterId) {
  ...
  const { ok: vOk, data: vData } = await apiFetch(`/videos?chapterId=${chapterId}`);
  if (!vOk) return;
  state.videos = vData.data;

  document.getElementById('videosTitle').textContent = state.currentChapter.name;
  document.getElementById('videosSubtitle').textContent = `${state.videos.length} video${state.videos.length !== 1 ? 's' : ''} curated for this chapter — watch in order for best results.`;

  const watchedIds = (state.progress.find(p => (p.chapterId?._id || p.chapterId) === chapterId) || {}).videoIds || [];
  ...
  if (state.currentChapter.nextSteps?.length) {
    ...
    nl.innerHTML = state.currentChapter.nextSteps.map(s => `<li>${s}</li>`).join('');
    const alreadyDone = state.progress.some(p => (p.chapterId?._id || p.chapterId) === chapterId && p.completed);
    if (alreadyDone) {
...
function openVideoModal(videoId) {
  ...
  document.getElementById('vModalMeta').textContent = `By ${v.creator} ${v.handle || ''} · ${(v.tags || []).join(', ')}`;
  document.getElementById('vModalLink').href = v.url;
  document.getElementById('vModalCredit').innerHTML = `<strong>🎖️ Credit:</strong> This video is created by <strong>${v.creator}</strong> (${v.handle || 'YouTube'}). EDU-Cations only curates — please visit their channel, like, and subscribe to support them!`;
  openModal('videoModal');
}

function closeVideoModalOutside(e) {
  if (e.target === document.getElementById('videoModal')) closeModal('videoModal');
}
...
async function markVideoWatched(videoId, chapterId) {
  if (!state.user) return;
  await apiFetch('/progress/video', { method: 'POST', body: JSON.stringify({ chapterId, videoId }) });
  fetchProgress();
}

async function markChapterComplete() {
  if (!state.user) { openModal('loginModal'); return; }
  const chapterId = document.getElementById('markCompleteBtn').dataset.chapterid;
  const { ok } = await apiFetch('/progress/complete', { method: 'POST', body: JSON.stringify({ chapterId }) });
  if (ok) {
...
async function login() {
  ...
  if (!ok) { showAuthError('loginError', data.message); return; }
...
async function register() {
  ...
  if (!name || !email || !password) { showAuthError('registerError', 'Please fill all required fields'); return; }
  if (password.length < 6) { showAuthError('registerError', 'Password must be at least 6 characters'); return; }
  const { ok, data } = await apiFetch('/auth/register', { method: 'POST', body: JSON.stringify({ name, email, password, class: cls || null }) });
  if (!ok) { showAuthError('registerError', data.message); return; }
  state.token = data.token;
  state.user = data.user;
  localStorage.setItem('edu_token', data.token);
  localStorage.setItem('edu_user', JSON.stringify(data.user));
...
function updateAuthUI() {
  ...
  if (state.user) {
    ...
    adminBtn.style.display = state.user.role === 'admin' ? 'inline-block' : 'none';
  } else {
...
function autoThumb(url) {
  ...
  if (match) document.getElementById('avThumb').value = `https://img.youtube.com/vi/${match[1]}/hqdefault.jpg`;
}
...
async function openAddVideo() {
  ...
  ['avTitle','avCreator','avHandle','avUrl','avThumb','avTags','avOrder'].forEach(id => document.getElementById(id).value = '');
...
async function submitVideo() {
  ...
  const body = {
    ...
    tags: document.getElementById('avTags').value.split(',').map(t => t.trim()).filter(Boolean),
    order: parseInt(document.getElementById('avOrder').value) || 1,
  };
  if (!body.title || !body.creator || !body.url || !body.chapterId) { showAuthError('addVideoError', 'Title, Creator, URL and Chapter are required'); return; }
  const method = id ? 'PUT' : 'POST';
  const path = id ? `/admin/videos/${id}` : '/admin/videos';
  const { ok, data } = await apiFetch(path, { method, body: JSON.stringify(body) });
  if (!ok) { showAuthError('addVideoError', data.message); return; }
...
async function deleteVideo(id) {
  ...
  if (ok) { showToast('Video deleted.', 'success'); loadAdminVideos(); }
...
async function openAddChapter() {
  ...
  ['acSubject','acName','acOrder','acNextSteps'].forEach(id => document.getElementById(id).value = '');
...
async function submitChapter() {
  ...
  const body = {
    ...
    nextSteps: document.getElementById('acNextSteps').value.split('\n').map(s => s.trim()).filter(Boolean),
  };
  if (!body.classId || !body.subject || !body.name) { showAuthError('addChapterError', 'Class, Subject and Name are required'); return; }
  const method = id ? 'PUT' : 'POST';
  const path = id ? `/admin/chapters/${id}` : '/admin/chapters';
  const { ok, data } = await apiFetch(path, { method, body: JSON.stringify(body) });
  if (!ok) { showAuthError('addChapterError', data.message); return; }
...
async function deleteChapter(id) {
  if (!confirm('Delete this chapter and ALL its videos? This cannot be undone.')) return;
  const { ok } = await apiFetch(`/admin/chapters/${id}`, { method: 'DELETE' });
  if (ok) { showToast('Chapter deleted.', 'success'); loadAdminChapters(); }
  else showToast('Failed to delete.', 'error');
}
...
async function populateChapterSelect(selectId, selectedId = '') {
  ...
  const { ok, data } = await apiFetch('/admin/chapters');
  if (!ok) return;
  sel.innerHTML = data.data.map(ch =>
...
async function populateClassSelect(selectId, selectedId = '') {
  ...
  const { ok, data } = await apiFetch('/classes');
  if (!ok) return;
  sel.innerHTML = data.data.map(cls =>
